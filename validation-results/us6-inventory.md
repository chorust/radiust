...
## Separate old gray display inventory — 2026-09-22

Task 002 inventories 25 provisional display paths across 22 source IDs;
these cover all presently known named region/tile catalog products plus
separately evidenced TH/MY subpaths and three additional legacy acquisition
paths. Each has an independent blocked entry under
`migration/sources/<source>.json:display_migration`, in the fixture manifest,
the packaged display registry index, and in
`validation-results/legacy-display.json`. The offline audit cross-checks
the path sets and status totals: **25 blocked, 0 passed**. This establishes
only the currently known *structural* coverage. The old commit/config and
product-override evidence remains unavailable, so the complete old display
path count and SC-008 are not yet verified. Per-source raw and scientific
statuses recorded below retain their original meanings; no display block or
synthetic match is a scientific acceptance or an accepted migration exception.
...
## CLI Experience Task 002 legacy display replay — 2026-09-23

The user supplied the local old-project snapshot at `/Users/blizhan/data/code/旧项目/旧项目-旧项目`, exact commit `8d251601ca551fbd5c05451f1fb337fc4b75362c`. Seven paths reproduce historical raw/gray pairs pixel-for-pixel: AU, ID, KR, MY peninsular, SG, TW observation, and VN CMAX. Seven more use frozen goldens generated by replaying the exact old code on legal migration-validation fixtures: CA, ES, NZ, FR, PT, TH `kkn240Loop`, and Royal Rain `takhli` using `THKL3.yaml`. Generated files are identified as replay artifacts, not historical old outputs. Hashes, crops, dimensions, rules and provenance are in [`legacy-display.json`](legacy-display.json).

The current in-scope comparison totals are **15 passed / 0 difference_pending / 8 blocked**. T037, T038, T040, T041, T042, T043, T044 and T045 are complete within their recorded evidence and accepted scope; T039 remains open for PH's absent matched pair. MY East uses `MY/base.yaml`, which matches all three retained archive maps and the current engine's 568×640 output pixel-exactly. The TH `cmp1` frame remains recorded as blocked but is outside T040's completion gate. The TW grid has no old image-display path; `tw-http` lacks endpoint-specific archive provenance. Six additional `TWCOMP3` raw/map pairs replay generic `TW/base.yaml` with zero pixel differences; the current station remains provider-native `CV1_3600`, and old aliases do not set it. RainViewer, Windy and BMKG display paths remain blocked pending future matching/source work. OpenSnow/WU were removed from legacy-display scope by user decision; their acquisition records, adapter and provider blockers remain unchanged. `id_sidarma/cmax` remains unbound. Old files were locally attested for migration validation only; official fixture terms and AEMET attribution remain attached to generated goldens. No redistribution claim is made. These display checks do not alter source science, geometry, credential, provider, or storage gates.


## Task 002 legacy-display follow-up — 2026-09-23

- MY East: all three locally authorized archive pairs replay exactly with `MY/base.yaml`; the current display engine also reproduces the registered 568×640 gray baseline with zero pixel differences. This is the selected configuration for the current `east` station; no explanation of the old route choice is needed. Evidence and hashes: `tests/fixtures/legacy-display/my/east/archive-candidates.json`. Current station identity is not derived from old project aliases.
- PH: no files under the old `output/ph`, no pair in the pinned snapshot, and no verified current raw were found. T039 remains open because replay and pixel comparison lack a source-matched pair.
- RainViewer/Windy: their old transform records and current-tile probes remain in the inventory. The user accepted T042/T043 as complete for this task; new matching work is deferred, and these paths remain blocked from automatic display conversion. The user identifies Windy red as radar and green as likely phase for the current task; this is recorded as the task-scope interpretation.
- BMKG: the public tile probe returned HTTP 403 with no retained frame. Per user decision T045 is complete for the present task; a future reachable source requires a new adapter/matching pass, and the path remains blocked.
- Scope decisions: T038, T040, T042, T043, T044 and T045 are complete; T039/PH is the only unchecked source batch. Overall comparison is **15 passed / 0 difference_pending / 8 blocked**; scientific status remains unchanged.
